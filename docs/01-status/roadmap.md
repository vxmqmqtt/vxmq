...
| M3-07 | Maximum Packet Size | 已完成 | Broker 在 CONNACK 声明 maximum packet size；入站 MQTT 5 PUBLISH 超限断连；出站超出客户端限制时跳过投递；测试覆盖 |
...
| M3-12 | Metrics（连接数 / 会话数 / 消息速率等） | 已完成 | `/q/metrics` 暴露连接数、会话数、Broker runtime state、订阅变更、消息路由和协议告警等低基数指标；消息速率由 Prometheus `rate()` 基于 counter 计算；端点验收测试覆盖 |
| M3-13 | 日志与诊断增强 | 已完成 | 关键协议事件输出稳定 `key=value` 诊断日志；CONNECT / SUBSCRIBE / UNSUBSCRIBE / PUBLISH 拒绝、断连、关闭、投递失败具备连接和 MQTT reason code 定位字段；文档补充排障入口 |

### M3 验收门槛

- `M3-01`、`M3-02`、`M3-03`、`M3-10`、`M3-11`、`M3-12`、`M3-13` 至少达到 `已完成`。
