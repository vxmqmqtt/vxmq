...
`M2` 完成订阅树 / 路由索引重构后的早期实现中，`InMemorySubscriptionRegistry` 仍然是共享可变树：
...
- 这类“写成本很夸张”的观感，还会被批量 load / unload 型 benchmark 放大，因为那类 workload 测的是整批构建与整批移除，不等于单次 `add/remove` 延迟。
...
- 当前主线已经进一步收敛为：
  - 用更紧凑的不可变 child / binding 容器减少小节点场景下的复制放大
  - 用内部 batch snapshot builder 支持整批订阅重建
  - 用独立评估套件区分“单次 churn 写入成本”和“整批 snapshot 重建成本”
...
- 当前结论状态：`已落地主线并持续优化中`，后续以 `M4` 性能基线验证为准继续做写放大与延迟治理。
